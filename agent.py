...
# ─── ADC-authenticated Gemini model ────────────────────────────


class GeminiADC(Gemini):
    """Gemini model variant that authenticates via Application Default Credentials."""

    @cached_property
    def api_client(self) -> google_genai.Client:
        credentials, detected_project = google.auth.default(
            scopes=["https://www.googleapis.com/auth/cloud-platform"]
        )
        project = GOOGLE_CLOUD_PROJECT or detected_project
...
async def main_async(args):
    ...
    log.info(f"   Model: {MODEL} (via Vertex AI / ADC)")
    log.info(f"   Project: {GOOGLE_CLOUD_PROJECT or '(auto-detected)'}")
